...
const historyKpiValueClass =
  'text-[1rem] font-black tabular-nums leading-none tracking-[-0.03em] text-[#111827] drop-shadow-sm sm:text-[1.125rem] lg:text-[1.25rem]';
...
function HistoryKpiContent({
  ...
  return (
    <div className={`relative z-10 flex w-full flex-col ${compact ? 'min-h-[4.25rem]' : 'min-h-[5rem]'}`}>
      <p className={historyKpiLabelClass}>{label}</p>
      <div className="mt-auto">
        {loading ? (
          <div
            className={`animate-pulse rounded-md bg-[#EBEBEB] ${
              compact ? 'h-[18px] w-[2.5rem] sm:h-5 sm:w-[2.75rem]' : 'h-6 w-[3.5rem]'
            }`}
          />
        ) : (
          <p
            className={
              compact
                ? 'text-[0.875rem] font-black tabular-nums leading-none tracking-[-0.03em] text-[#111827] drop-shadow-sm sm:text-[0.9375rem]'
        ...
        <div className="mt-1 flex min-h-[1rem] items-center">{trendBadge}</div>
      </div>
    </div>
  );
}
...
function MobileStat({
  ...
  return (
    <div
      className={`relative flex min-h-[4.75rem] min-w-0 flex-1 overflow-hidden rounded-[12px] p-2.5 sm:min-h-[5rem] sm:rounded-[14px] sm:p-3 ${
...
function DesktopStatBlock({
  ...
  return (
    <article className={`${apptHistoryKpiTile} h-full min-h-[5rem]`}>
    ...
    <>
      {mobileHeader ? (
        <section className="overflow-hidden rounded-[16px] bg-white ring-1 ring-[#EEEEEE] lg:hidden">
          ...
          <div className="px-4 pb-3 pt-2.5 sm:pb-3.5">
      ...
      <div className="hidden min-w-0 flex-1 lg:grid lg:grid-cols-3 lg:items-stretch lg:gap-3">
